...
class ParseInputStream {
  ...
  private function file_stream($data) {
    ...
    if ($idx === false) {
      Log::warning(
        "ParseInputStream.file_stream(): Could not locate header separator in data:"
      );
      Log::warning($data);
    } else {
      ...
      while ($header !== false) {
        if (
          substr($header, 0, strlen("Content-Disposition: ")) ==
          "Content-Disposition: "
        ) {
          ...
          substr($header, 0, strlen("Content-Type: ")) == "Content-Type: "
        ) {
  ...
  private function parameter($string) {
    ...
    if (
      ...
      if (preg_match('/^(.*)\[\]$/i', $match[1], $tmp)) {
        $data[$tmp[1]][] =
          isset($match[2]) && $match[2] !== null ? $match[2] : null;
      } else {
        $data[$match[1]] =
          isset($match[2]) && $match[2] !== null ? $match[2] : null;
      }
  ...
  function parse_parameter(&$params, $parameter, $value) {
    if (strpos($parameter, '[') !== false) {
      ...
      if (preg_match('/^([^[]*)\[([^]]*)\](.*)$/', $parameter, $match)) {
        ...
        if ($name !== '' && $name !== null) {
          ...
          if (strlen($rem) > 0) {
            if ($key === '' || $key === null) {
              ...
              if (
                !isset($params[$name][$key]) ||
                !is_array($params[$name][$key])
              ) {
                $params[$name][$key] = [];
              }
        ...
        Log::warning(
          "ParseInputStream.parse_parameter() Parameter name regex failed: '" .
            $parameter .
            "'"
        );
      }
